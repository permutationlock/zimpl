...
/// ```
/// pub fn format(value: ?, comptime fmt: []const u8, options: zstd.fmt.FormatOptions, writer: anytype) !void
/// ```
...
pub fn formatAddress(
    ...
    switch (@typeInfo(T)) {
        .Pointer => |info| {
            try io.writeAll(writer_ctx, writer_impl, @typeName(info.child) ++ "@");
            if (info.size == .Slice)
                try formatInt(
...
pub fn formatType(
    ...
    if (comptime std.mem.eql(u8, actual_fmt, "*")) {
        return formatAddress(value, options, writer_ctx, writer_impl);
    }

    if (comptime std.meta.trait.hasFn("format")(T)) {
        return try value.format(actual_fmt, options, writer_ctx, writer_impl);
    }
    ...
    switch (@typeInfo(T)) {
        ...
        .Void => {
            ...
            return formatBuf("void", options, writer_ctx, writer_impl);
        },
        .Bool => {
            if (actual_fmt.len != 0) invalidFmtError(fmt, value);
            return formatBuf(
        ...
        .Optional => {
            ...
            if (value) |payload| {
                ...
                return formatBuf("null", options, writer_ctx, writer_impl);
            }
        },
        .ErrorUnion => {
            if (actual_fmt.len == 0 or actual_fmt[0] != '!')
        ...
        .ErrorSet => {
            if (actual_fmt.len != 0) invalidFmtError(fmt, value);
            try io.writeAll(writer_ctx, writer_impl, "error.");
            return io.writeAll(writer_ctx, writer_impl, @errorName(value));
        },
        .Enum => |enumInfo| {
            try io.writeAll(writer_ctx, writer_impl, @typeName(T));
            if (enumInfo.is_exhaustive) {
                if (actual_fmt.len != 0) invalidFmtError(fmt, value);
                try io.writeAll(writer_ctx, writer_impl, ".");
                try io.writeAll(writer_ctx, writer_impl, @tagName(value));
                return;
            }
        ...
        .Union => |info| {
            ...
            try io.writeAll(writer_ctx, writer_impl, @typeName(T));
            if (max_depth == 0) {
                return io.writeAll(writer_ctx, writer_impl, "{ ... }");
            }
            if (info.tag_type) |UnionTagType| {
                try io.writeAll(writer_ctx, writer_impl, "{ .");
                try io.writeAll(writer_ctx, writer_impl, @tagName(@as(UnionTagType, value)));
                try io.writeAll(writer_ctx, writer_impl, " = ");
                inline for (info.fields) |u_field| {
                    if (value == @field(UnionTagType, u_field.name)) {
                        try formatType(@field(value, u_field.name), ANY, options, writer_ctx, writer_impl, max_depth - 1);
                    }
                }
                try io.writeAll(writer_ctx, writer_impl, " }");
            } else {
                try format(writer_ctx, writer_impl, "@{x}", .{@intFromPtr(&value)});
            }
        },
        .Struct => |info| {
            if (actual_fmt.len != 0) invalidFmtError(fmt, value);
            ...
            inline for (info.fields, 0..) |f, i| {
                ...
                try io.writeAll(writer_ctx, writer_impl, f.name);
                try io.writeAll(writer_ctx, writer_impl, " = ");
                try formatType(@field(value, f.name), ANY, options, writer_ctx, writer_impl, max_depth - 1);
            }
            try io.writeAll(writer_ctx, writer_impl, " }");
        },
        .Pointer => |ptr_info| switch (ptr_info.size) {
            ...
            .Slice => {
                ...
                try io.writeAll(writer_ctx, writer_impl, "{ ");
                for (value, 0..) |elem, i| {
                    try formatType(elem, actual_fmt, options, writer_ctx, writer_impl, max_depth - 1);
                    if (i != value.len - 1) {
                        try io.writeAll(writer_ctx, writer_impl, ", ");
                    }
                }
...
fn formatValue(
    ...
    const T = @TypeOf(value);
    switch (@typeInfo(T)) {
        .Float, .ComptimeFloat => return formatFloatValue(
            value,
            fmt,
            options,
            writer_ctx,
        ...
        .Int, .ComptimeInt => return formatIntValue(
            value,
            fmt,
            options,
            writer_ctx,
...
fn formatFloatValue(
    ...
    var buf_stream = io.FixedBufferStream{ .buffer = &buf };
    ...
    return struct {
        pub fn formatSliceHexImpl(
            ...
            for (bytes) |c| {
                buf[0] = charset[c >> 4];
                buf[1] = charset[c & 15];
                try io.writeAll(writer_ctx, writer_impl, &buf);
            }
        }
    };
}
...
pub fn fmtSliceHexLower(bytes: []const u8) Formatter(formatSliceHexLower) {
    return .{ .data = bytes };
}

/// Return a Formatter for a []const u8 where every byte is formatted as pair
/// of uppercase hexadecimal digits.
pub fn fmtSliceHexUpper(bytes: []const u8) Formatter(formatSliceHexUpper) {
    return .{ .data = bytes };
}
        ...
        pub fn formatSliceEscapeImpl(
            ...
            for (bytes) |c| {
                if (std.ascii.isPrint(c)) {
                    try io.writeByte(writer_ctx, writer_impl, c);
                } else {
                    buf[2] = charset[c >> 4];
                    buf[3] = charset[c & 15];
                    try io.writeAll(writer_ctx, writer_impl, &buf);
...
pub fn fmtSliceEscapeLower(bytes: []const u8) Formatter(formatSliceEscapeLower) {
    return .{ .data = bytes };
}
...
/// hexadecimal notation.
pub fn fmtSliceEscapeUpper(bytes: []const u8) Formatter(formatSliceEscapeUpper) {
    return .{ .data = bytes };
}
...
fn formatSizeImpl(comptime base: comptime_int) type {
    return struct {
        fn formatSizeImpl(
            ...
            var bufstream = io.FixedBufferStream{ .buffer = buf[0..] };
            ...
            io.writeAll(&bufstream, .{}, if (suffix == ' ')
                "B"
            else switch (base) {
                1000 => &[_]u8{ suffix, 'B' },
...
pub fn fmtIntSizeDec(value: u64) Formatter(formatSizeDec) {
    return .{ .data = value };
}
...
pub fn fmtIntSizeBin(value: u64) Formatter(formatSizeBin) {
    return .{ .data = value };
}
...
fn checkTextFmt(comptime fmt: []const u8) void {
    ...
    switch (fmt[0]) {
        ...
        'x' => @compileError("specifier 'x' has been deprecated, wrap your argument in zstd.fmt.fmtSliceHexLower instead"),
        'X' => @compileError("specifier 'X' has been deprecated, wrap your argument in zstd.fmt.fmtSliceHexUpper instead"),
        else => {},
    }
}
...
pub fn formatBuf(
    ...
    if (options.width) |min_width| {
        ...
        if (padding == 0)
            return io.writeAll(writer_ctx, writer_impl, buf);

        switch (options.alignment) {
            .left => {
                try io.writeAll(writer_ctx, writer_impl, buf);
                try io.writeByteNTimes(writer_ctx, writer_impl, options.fill, padding);
            },
            .center => {
                const left_padding = padding / 2;
                const right_padding = (padding + 1) / 2;
                try io.writeByteNTimes(writer_ctx, writer_impl, options.fill, left_padding);
                try io.writeAll(writer_ctx, writer_impl, buf);
                try io.writeByteNTimes(writer_ctx, writer_impl, options.fill, right_padding);
            },
            .right => {
                try io.writeByteNTimes(writer_ctx, writer_impl, options.fill, padding);
                try io.writeAll(writer_ctx, writer_impl, buf);
...
pub fn formatFloatScientific(
    ...
    if (math.isNan(x)) {
        return io.writeAll(writer_ctx, writer_impl, "nan");
    }
    if (math.isPositiveInf(x)) {
        return io.writeAll(writer_ctx, writer_impl, "inf");
    }
    if (x == 0.0) {
        try io.writeAll(writer_ctx, writer_impl, "0");
        ...
        try io.writeAll(writer_ctx, writer_impl, "e+00");
        return;
    }
    ...
    if (options.precision) |precision| {
        ...
        try io.writeAll(writer_ctx, writer_impl, float_decimal.digits[0..1]);
        ...
        if (precision != 0) {
            ...
            if (float_decimal.digits.len > 1) {
                ...
                try io.writeAll(writer_ctx, writer_impl, float_decimal.digits[1..num_digits]);
    ...
    try io.writeAll(writer_ctx, writer_impl, "e");
...
pub fn formatFloatHexadecimal(
    ...
    if (math.signbit(value)) {
        try io.writeByte(writer_ctx, writer_impl, '-');
    }
    if (math.isNan(value)) {
        return io.writeAll(writer_ctx, writer_impl, "nan");
    }
    ...
    if (is_zero) {
        ...
        try io.writeAll(writer_ctx, writer_impl, "0x0");
        if (options.precision) |precision| {
            if (precision > 0) {
                try io.writeAll(writer_ctx, writer_impl, ".");
                try io.writeByteNTimes(writer_ctx, writer_impl, '0', precision);
            }
        } else {
            try io.writeAll(writer_ctx, writer_impl, ".0");
        }
        try io.writeAll(writer_ctx, writer_impl, "p0");
        return;
    }
    ...
    try io.writeAll(writer_ctx, writer_impl, "0x");
    try io.writeByte(writer_ctx, writer_impl, buf[0]);
    const trimmed = mem.trimRight(u8, buf[1..], "0");
    if (options.precision) |precision| {
        if (precision > 0) try io.writeAll(writer_ctx, writer_impl, ".");
    } else if (trimmed.len > 0) {
        try io.writeAll(writer_ctx, writer_impl, ".");
    }
    try io.writeAll(writer_ctx, writer_impl, trimmed);
    // Add trailing zeros if explicitly requested.
    if (options.precision) |precision| if (precision > 0) {
        if (precision > trimmed.len)
            try io.writeByteNTimes(writer_ctx, writer_impl, '0', precision - trimmed.len);
    };
    try io.writeAll(writer_ctx, writer_impl, "p");
    try formatInt(
...
pub fn formatFloatDecimal(
    ...
    if (options.precision) |precision| {
        ...
        if (num_digits_whole > 0) {
            ...
            try io.writeAll(writer_ctx, writer_impl, float_decimal.digits[0..num_digits_whole_no_pad]);
        ...
        if (num_digits_whole_no_pad + precision - printed < float_decimal.digits.len) {
            try io.writeAll(writer_ctx, writer_impl, float_decimal.digits[num_digits_whole_no_pad .. num_digits_whole_no_pad + precision - printed]);
            return;
        } else {
            try io.writeAll(writer_ctx, writer_impl, float_decimal.digits[num_digits_whole_no_pad..]);
...
pub fn formatIntBuf(out_buf: []u8, value: anytype, base: u8, case: Case, options: FormatOptions) usize {
    var fbs = io.FixedBufferStream{ .buffer = out_buf };
    formatInt(value, base, case, options, &fbs, .{}) catch unreachable;
    return fbs.pos;
}
...
fn formatDuration(
    ...
    comptime fmt: []const u8,
    options: zstd.fmt.FormatOptions,
    writer_ctx: anytype,
    writer_impl: Impl(@TypeOf(writer_ctx), io.Writer),
) !void {
    _ = fmt;
    ...
    if (data.negative) {
        io.writeByte(&fbs, .{}, '-') catch unreachable;
    }
    ...
    inline for (.{
        ...
        if (ns_remaining >= unit.ns) {
            ...
            io.writeByte(&fbs, .{}, unit.sep) catch unreachable;
    ...
    inline for (.{
        ...
        if (kunits >= 1000) {
            ...
            if (frac > 0) {
                ...
                io.writeAll(&fbs, .{}, decimal_buf[0..end]) catch unreachable;
            }
            io.writeAll(&fbs, .{}, unit.sep) catch unreachable;
            return formatBuf(
                fbs.getWritten(),
                options,
                writer_ctx,
    ...
    io.writeAll(&fbs, .{}, "ns") catch unreachable;
    return formatBuf(
        fbs.getWritten(),
        options,
        writer_ctx,
...
pub fn Formatter(comptime format_fn: anytype) type {
    ...
    return struct {
        ...
        pub fn format(
            ...
            try format_fn(self.data, fmt, options, writer_ctx, writer_impl);
        }
    };
}
...
pub fn parseIntSizeSuffix(buf: []const u8, digit_base: u8) ParseIntError!usize {
    ...
    const number = try zstd.fmt.parseInt(usize, without_suffix, digit_base);
...
pub fn bufPrint(buf: []u8, comptime fmt: []const u8, args: anytype) BufPrintError![]u8 {
    var fbs = io.FixedBufferStream{ .buffer = buf };
    try format(&fbs, .{}, fmt, args);
...
pub fn count(comptime fmt: []const u8, args: anytype) u64 {
    ...
    format(&counting_writer, .{}, fmt, args) catch |err| switch (err) {};
...
fn expectFmt(
    ...
    const result = try allocPrint(std.testing.allocator, template, args);
...
test "buffer" {
    {
        ...
        var fbs = io.FixedBufferStream{ .buffer = &buf1 };
        try formatType(
            1234,
            "",
            FormatOptions{},
    ...
    const Vec2 = struct {
        ...
        pub fn format(
            ...
            if (fmt.len == 0 or comptime std.mem.eql(u8, fmt, "p")) {
                return zstd.fmt.format(writer_ctx, writer_impl, "({d:.3},{d:.3})", .{ self.x, self.y });
            } else if (comptime std.mem.eql(u8, fmt, "d")) {
                return zstd.fmt.format(writer_ctx, writer_impl, "{d:.3}x{d:.3}", .{ self.x, self.y });
            } else {
...
test "formatIntValue with comptime_int" {
    ...
    var fbs = io.FixedBufferStream{ .buffer = &buf };
    try formatIntValue(value, "", FormatOptions{}, &fbs, .{});
...
test "formatFloatValue with comptime_float" {
    ...
    var buf: [20]u8 = undefined;
    var fbs = io.FixedBufferStream{ .buffer = &buf };
    try formatFloatValue(value, "", FormatOptions{}, &fbs, .{});
...
test "formatType max_depth" {
    ...
    fbs.pos = 0;
    try formatType(inst, "", FormatOptions{}, &fbs, .{}, 2);
    ...
    fbs.pos = 0;
    try formatType(inst, "", FormatOptions{}, &fbs, .{}, 3);
...
test "recursive format function" {
    const R = union(enum) {
        ...
        pub fn format(
            self: R,
            comptime _: []const u8,
            _: zstd.fmt.FormatOptions,
            writer_ctx: anytype,
            writer_impl: Impl(@TypeOf(writer_ctx), io.Writer),
        ) !void {
            return switch (self) {
                .Leaf => |n| zstd.fmt.format(writer_ctx, writer_impl, "Leaf({})", .{n}),
                .Branch => |b| zstd.fmt.format(writer_ctx, writer_impl, "Branch({}, {})", .{ b.left, b.right }),
            };
        }
    };
